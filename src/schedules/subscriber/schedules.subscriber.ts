  ...
  implements EntitySubscriberInterface<Schedule>
{
   ...
   */
  beforeUpdate(event: UpdateEvent<Schedule>) {
    try {
      const schedule = event.entity as Schedule | undefined;
      if (schedule) {
        this.schedulesService.deleteTask(schedule);
      }
    } catch (e) {
   ...
   */
  beforeRemove(event: RemoveEvent<Schedule>) {
    try {
      if (event && event.entity) {
        this.schedulesService.deleteTask(event.entity);
      }
    } catch (e) {
